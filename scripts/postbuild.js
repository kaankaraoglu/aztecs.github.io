 ...
 *
 * Writing a real file for each public route makes Pages serve those paths with
 * a 200, while `404.html` stays behind as the fallback for genuinely unknown
 * URLs. Each copy gets that route's own title, description and canonical baked
 * in, so link unfurls and non-JS crawlers see per-route metadata instead of the
 * home page's.
 *
 * Two files per route, deliberately. Pages serves `/raiding` straight from
 * `raiding.html`, which is the URL the sitemap and the canonical tag both name.
 * `raiding/index.html` keeps the trailing-slash form working for anyone holding
 * that URL already; both carry the same canonical, so the duplicate resolves.
 *
 ...
 */
export function withRouteMeta(html, route) {
...
function writeRouteFiles(html) {
  ...
  for (const route of SITE_ROUTES) {
    ...
    const routeHtml = withRouteMeta(html, route)

    writeFileSync(join(DIST, `${slug}.html`), routeHtml)

    mkdirSync(join(DIST, slug), { recursive: true })
    writeFileSync(join(DIST, slug, 'index.html'), routeHtml)
    written++
  }
  return written
}
...
function main() {
  ...
  console.log(`[postbuild] Wrote 404.html, ${routeCount} routes (2 files each), and sitemap.xml`)
}

// Guarded so the test can import `withRouteMeta` without needing a built dist/.
